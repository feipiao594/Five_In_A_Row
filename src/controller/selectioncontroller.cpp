...
void SelectionController::startLocal(bool blackIsComputer, bool whiteIsComputer) {
  ...
  QObject::disconnect(boardController, &BoardController::boardClicked, nullptr,
                      nullptr);

  auto black = blackIsComputer
                   ? static_cast<Player *>(new AiPlayer(Unit::Black, mgr, mgr))
                   : static_cast<Player *>(new HumanPlayer(Unit::Black, mgr, mgr));
  auto white = whiteIsComputer
                   ? static_cast<Player *>(new AiPlayer(Unit::White, mgr, mgr))
  ...
  if (auto human = qobject_cast<HumanPlayer *>(black))
    connect(boardController, &BoardController::boardClicked, human,
            &HumanPlayer::onBoardClick, Qt::UniqueConnection);
