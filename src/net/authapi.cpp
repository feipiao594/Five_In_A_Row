  ...
  auto doRefresh = [this](const QString &rt) {
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
      ...
      if (err.error != QJsonParseError::NoError || !doc.isObject()) {
        AuthStore::clearLogin();
        sessionCheckInFlight_ = false;
        emit sessionChecked(false, "无法刷新登录：返回格式错误");
        return;
      }
      auto obj = doc.object();
      ...
      if (httpStatus < 200 || httpStatus >= 300) {
        ...
        sessionCheckInFlight_ = false;
        emit sessionChecked(false, msg.isEmpty() ? "登录已失效，请重新登录" : msg);
        return;
      }
      ...
      AuthStore::saveLogin(u, at, newRt.isEmpty() ? AuthStore::refreshToken() : newRt);
      sessionCheckInFlight_ = false;
