...
int Match::totalStep() const { return (int)record.size(); }

Unit Match::curColor() const {
  return ((totalStep() & 1) == 0) ? Unit::Black : Unit::White;
}

Unit Match::winner() const { return winner_; }

Coordinate Match::latestCoord() const { return record.top(); }

QVector<Coordinate> Match::undoList() const { return undoList_; }

bool Match::isOver() const { return over_; }
...
void Match::drop(Coordinate coord) {
  ...
  // 先更新对局状态，再发信号，避免上层在 onDropped 时误以为还能继续走下一手（和局时会触发 AI 选点崩溃）。
  if (isWin(coord)) {
    winner_ = unit;
    over_ = true;
